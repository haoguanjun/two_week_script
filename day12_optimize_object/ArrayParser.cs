...
namespace week2
{
    public class ArrayParser
    {
        ...
        public void Init()
        {
            ...
            // 在支持 Array 的情况下，增加一种，成为 6 种
            Parser elements = Parser.Rule(typeof(ArrayLiteral))
                .Ast(expr)
                .Repeat(Parser.Rule().Sep(",").Ast(expr));
            primary.InsertChoice(
                Parser.Rule()
                    .Sep("[")
                    .Maybe(elements)
            ...
            // class
            Parser member = Parser.Rule()
                    .Or(
                        def,
                        simple
                        );
            Parser class_body = Parser.Rule(typeof(ClassBody))
                    .Sep("{")
                    .Option(member)
                    .Repeat(
                        Parser.Rule()
                    ...
                    .Sep("}");
            Parser defClass = Parser.Rule(typeof(ClassStmnt))
                    .Sep("class")
                    .Identifier(reserved)
                    .Option(Parser.Rule().Sep("extends").Identifier(reserved))
                    .Ast(class_body);
            postfix.InsertChoice(Parser.Rule(typeof(Dot))
                        .Sep(".")
                        .Identifier(reserved));

            // 对于数组来说，增加一种后缀形式
            postfix.InsertChoice(Parser.Rule(typeof(ArrayRef))
                    .Sep("[")
                    .Ast(expr)
                    .Sep("]")
                    );
