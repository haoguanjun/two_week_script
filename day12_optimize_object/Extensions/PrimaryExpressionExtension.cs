...
namespace week2
{
    public static class PrimaryExpressionExtension
    {
            ...
            if (HasPostfix(primary, nest))
            {
                ...
                switch (postfix)
                {
                    case Arguments arguments:
                        ...
                        break;
                    case Dot dot:
                        result = dot.Eval(environment, target);
                        break;
                    case ArrayRef arrayRef:
                        result = arrayRef.Eval(environment, target);
                        break;
                }
                return result;
            }
        ...
        public static bool HasPostfix(this PrimaryExpr primary, int next)
        {
            return primary.Count - next > 1;
        }

        // 获取后缀
        public static Postfix Postfix(this PrimaryExpr primary, int next)
        {
